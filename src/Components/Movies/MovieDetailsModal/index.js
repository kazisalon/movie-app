...
const MovieDetailsModal = props => {
  const { loading, error, toggleModal, isModalOpen } = props;
  let { details } = props;
  const formatedGenres = details.genres ? details.genres.map(genre => genre.name) : [];
          ...
          <ModalCard backgroundImage={`${details.backdrop_path}`} error={error}>
            ...
            {error ? (
              <LoadingError>
                {' '}
                Failed to load =( <br />
                На улице рейн на душе пейн
              </LoadingError>
            ) : (
              <DescriptionWrapper>
                <MovieTitle>
                  {`${details.title}`}
                  {details.vote_average !== 0 && <Circle>{details.vote_average}</Circle>}
                ...
                {formatedGenres.length >= 1 && (
                  <DesctiptionSection>
                    <DescriptionTitle>Genres </DescriptionTitle>{' '}
                    <MovieGenres>{formatedGenres.join(' • ')}</MovieGenres>
                  </DesctiptionSection>
                )}
